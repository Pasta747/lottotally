#!/usr/bin/env node
import { TwitterApi } from '/root/PastaOS/tools/x-api/node_modules/twitter-api-v2/dist/esm/index.js';
import fs from 'fs';
...
const ACCOUNTS = {
  pingerhq: {
    ...
    followUsernames: ['webflow','framer','vercel','netlify','cloudflare','render','railway','sentry','datadoghq','posthog','supabase','neondatabase','uptimerobot','betterstackhq','indiehackers','levelsio','shl','tinyseed','lauraroeder','transistorfm','convertkit','nathanbarry']
  },
  canopyfilter: {
    ...
    followUsernames: ['shepodcasts','descriptapp','riversidedotfm','buzzsprout','captivatefm','podmatch_com','transistorfm','convertkit','kit','substackinc','patreon','beaconsai','stanstore','uscreen','circle','thinkific','teachable','nathanbarry','lijin18','cathyheller','amyporterfield','JennaKutcher']
  }
};

const account = process.argv[2];
if (!ACCOUNTS[account]) {
  console.error('usage: node tmp_x_launch.mjs <account>');
  process.exit(1);
}
...
async function main() {
  try {
    ...
    out.limitations.push(`updateAccountProfile failed: ${e.code || ''} ${e.data?.detail || e.message}`.trim());
  }
  ...
  try {
    ...
    out.limitations.push(`updateAccountProfileImage failed: ${e.code || ''} ${e.data?.detail || e.message}`.trim());
  }
  ...
  try {
    ...
    out.limitations.push(`updateAccountProfileBanner failed: ${e.code || ''} ${e.data?.detail || e.message}`.trim());
  }
  ...
  for (const username of cfg.followUsernames) {
    try {
      ...
      if (targetId === cfg.userId) {
        out.follows.skipped.push({ username, reason: 'self' });
        continue;
      }
      const res = await client.v2.follow(cfg.userId, targetId);
      if (res?.data?.following) out.follows.followed.push({ username, id: targetId });
      else out.follows.skipped.push({ username, reason: JSON.stringify(res?.data || {}) });
    } catch (e) {
      out.follows.failed.push({ username, error: e.data?.detail || e.message || String(e) });
    }
  }
