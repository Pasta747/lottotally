...
const features = [
  { title: "Inventory tracking", body: "Auto-reconcile packs sold vs. expected inventory so errors surface before they become losses.", icon: "📦" },
  { title: "Theft detection", body: "Flag suspicious discrepancies by shift, employee, and terminal with instant anomaly alerts.", icon: "🛡️" },
  { title: "Commission verification", body: "Catch incorrect commission rates before they cost you money — every payout, every cycle.", icon: "💰" },
  { title: "Daily snapshots", body: "See your short, pack, and variance numbers at a glance without logging into state portals.", icon: "📊" },
  { title: "Smart alerts", body: "Get notified the moment a pack goes missing, a rate changes, or a threshold is breached.", icon: "🚨" },
  { title: "CSV import", body: "Drop your daily reports and reconcile in seconds. No manual entry. No spreadsheets.", icon: "📥" },
];

const faqs = [
  { q: "How does the free trial work?", a: "Start with a 14-day free trial on the Pro plan. No credit card required. After the trial, choose the plan that fits your operation." },
  { q: "Can I use LottoTally for multiple stores?", a: "Yes. The Multi plan supports up to 20 locations. Each store gets its own terminal inventory tracking and consolidated reporting." },
  { q: "What lottery types does it support?", a: "Currently supports scratch-off and terminal (pull-tab) lottery tracking. We're evaluating instant ticket and sports betting additions." },
  { q: "How do I import my sales data?", a: "Upload a CSV from your state lottery portal directly in the dashboard. You can also photograph your daily report and let OCR do the work." },
  { q: "Is my data secure?", a: "Yes. Your data is encrypted in transit and at rest. We never sell or share your business data with third parties." },
];
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pb-20 md:pb-12">
      {/* Navbar */}
          ...
          <div className="mt-12 space-y-4">
            {faqs.map((faq, i) => (
              <div key={i} className="rounded-xl border border-white/10 bg-white/5">
                ...
                {openFaq === i && <div className="border-t border-white/10 px-5 pb-5 pt-3 text-sm text-slate-400">{faq.a}</div>}
              </div>
            ))}
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 bg-slate-900/95 backdrop-blur-md md:hidden">
        <div className="flex items-center justify-around py-2">
          <Link href="/" className="flex flex-col items-center gap-0.5 px-3 py-1 text-xs text-white">
          ...
          <Link href="/dashboard/daily" className="flex flex-col items-center gap-0.5 px-3 py-1 text-xs text-slate-400 hover:text-white">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
            <span>Daily</span>
          </Link>
          <Link href="/dashboard/scratch-offs" className="flex flex-col items-center gap-0.5 px-3 py-1 text-xs text-slate-400 hover:text-white">
          ...
          <Link href="/dashboard/commissions" className="flex flex-col items-center gap-0.5 px-3 py-1 text-xs text-slate-400 hover:text-white">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            <span>Commissions</span>
          </Link>
          <Link href="/dashboard/settings" className="flex flex-col items-center gap-0.5 px-3 py-1 text-xs text-slate-400 hover:text-white">
